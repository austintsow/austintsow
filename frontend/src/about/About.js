...
function About() {
    const text = [
        "i'm a builder at heart, always drawn to the intersection of technology and problem-solving. my journey started in sammamish, just thirty minutes east from seattle, where i grew up surrounded by a mix of innovation and nature. now, i focus on ai-driven applications, blending machine learning with product design to create intuitive, efficient solutions. outside of software, i'm deeply interested in web3, particularly in decentralized finance (defi) and solana's role in building a more scalable, permissionless financial ecosystem. the idea of rearchitecting trust and ownership on-chain is something i continue to explore.",
    ...
    const generateWordDelays = (words) => {
        ...
        for (let i = 0; i < words.length; i += batchSize) {
            shuffledIndexes.slice(i, i + batchSize).forEach(index => {
                wordDelays[index] = delay; // Assign delay to the shuffled indexes
            });
            delay += 0.5; // Increase delay between batches
            batchSize = Math.min(batchSize * 2, 16); // Double the batch size up to a max of 16
        }
        return wordDelays;
    };
                        ...
                        return (
                            <motion.p key={pIndex} style={{ marginBottom: "20px" }}>
                                {words.map((word, wIndex) => (
                                    <motion.span
                                        key={wIndex}
                                        initial={{ color: "rgba(128, 128, 128, 1)" }} // Light gray
                                        animate={{ color: "rgba(0, 0, 0, 1)" }} // Fade to black
