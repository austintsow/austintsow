...
import "./FestiveEvening.css";
import ArchiveBack from "./ArchiveBack";

/*
 * a festive evening together
 *
 * Restored verbatim from git 3fe3ad5 (2025-12-18). This page used to be
 * Gloria.js itself and was overwritten when that became the passcode gate, so
 * it only existed in history. Wrapped in .festive-page because its stylesheet
 * shares class names with the other restored card and the live gate.
 */

export const FESTIVE_PASSWORD = 'austinlovesgloria';

function FestiveEvening() {
    ...
    const handlePasswordSubmit = (e) => {
        e.preventDefault();
        if (password.toLowerCase() === FESTIVE_PASSWORD) {
            setShowError(false);
            setShowHearts(true);
            setTimeout(() => {
                setShowDetails(true);
